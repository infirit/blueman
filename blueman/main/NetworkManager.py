...
class NMConnectionBase(object):
    ...
    def __init__(self, bdaddr, error_handler=None, reply_handler=None):
        ...
        self.Config = Config('org.blueman.gsmsetting', '/org/blueman/gsmsettings/%s/' % bdaddr)
        # temporary for testing use bluez.Manager to get device object.
        self.Manager = Manager()
        self.device = self.Manager.find_device(self.bdaddr)
    ...
    def _on_connection_activate(self, client, result):
        try:
    ...
    def activate(self):
        ...
        self.client.activate_connection_async(self.connection, device, None, None, self._on_connection_activate)

    def deactivate(self):
        if self.active_connection:
            self.client.deactivate_connection(self.active_connection)
            self.active_connection = None
